// server.js
// ===============================
// Phoenix Clinic - Backend Server (Railway-ready, Hardened)
// ===============================
 ...
 * PUPPETEER_EXECUTABLE_PATH=/usr/bin/chromium (على Railway)
 */
...
// مهلة بروتوكول كروم (تمنع Runtime.callFunctionOn timed out)
...
// نسخة بمهلة قصوى لتفادي التعليق للأبد
...
// حوّل الأرقام العربية إلى إنجليزية قبل أي معالجة أخرى
...
function phonesEqual05(a,b){
  ...
  return A && B && A === B; // strict equality in 05xxxxxxxx format
}
...
async function searchAndOpenPatientByName(page, fullName) {
  ...
  // انتظر ظهور الاقتراحات (حتى 20 ثانية) مع ضخّ أحداث إضافية
...
async function readIdentityStatus(page, fileId) {
  ...
  const ssnVal = await page.evaluate(()=>{
    ...
    for(const td of tds){
      ...
      if(/^05\d{8}$/.test(digits)) continue; // استبعد الجوال
...
app.post('/api/create-patient', async (req, res) => {
  // مهلة إجمالية 60 ثانية لضمان عدم التعليق
  ...
  const handler = (async ()=>{
    try{
      ...
      let account=null;
      try{
        // استخدم نسخة بمهلة لتفادي الانتظار للأبد
        ...
        // تأكيد جاهزية الحقول قبل الكتابة
        ...
        // قد لا يحدث تنقل إن كان الحفظ سريعًا؛ لذلك:
        await page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 30000 }).catch(()=>{});
        // فحص ثانٍ متأخر قليلًا لاكتشاف التحذير لو ظهر بعد الإرسال
        await sleep(1500);
        if (await isDuplicatePhoneWarning(page)) {
          await browser.close(); if(account) releaseAccount(account);
          return res.json({
