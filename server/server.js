...
// الحسابات الأربعة (تأكد من تحديثها)
const ACCOUNTS = [
...
app.post('/api/times', async (req, res) => {
  ...
  try {
    ...
    console.error('Error in /api/times:', err);
    res.json({ times: [] });
  }
});
...
async function bookAppointment({ name, phone, clinic, month, time, account }) {
  ...
  try {
    await page.goto('https://phoenix.imdad.cloud/medica13/login.php?a=1', { waitUntil: 'networkidle2' });
    await page.$eval('input[name="username"]', (el, val) => el.value = val, account.user);
    await page.$eval('input[name="password"]', (el, val) => el.value = val, account.pass);
    await Promise.all([
      page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 20000 }),
      page.click('#submit')
    ]);
