...
if (cachedPrefetch && Array.isArray(cachedPrefetch.times)) {
  ...
  const filtered = applyClinicRulesToTimes(
    cachedPrefetch.times,
    clinicStr,
    effectivePeriod,
    null,        // ⛔ لا قواعد ثابتة
    clinics      // ✅ from / to من لوحة الموظف
  );

  return res.json({
    times: filtered,
    cached: true,
    source: 'prefetch+filtered'
  });
}
...
const clinics = readClinics();
const cfg = clinics.find(c => c.value === clinic);

if (!cfg) {
  return res.json({
    success: false,
    message: 'العيادة غير مفعلة'
  });
}
...
if (minutes < fromMin || minutes > toMin) {
  return res.json({
    success: false,
    message: 'الموعد خارج أوقات الدوام المحددة'
  });
}
