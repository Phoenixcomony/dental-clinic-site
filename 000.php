    ...
    *{ box-sizing:border-box; }
    ...
    .lux-bg{
      ...
      background:
        ...
        url('images/حجز الموعد.png') center/cover no-repeat;
    ...
    /* تقليل البلور على الجوال لو ثقيل */
...
<body>
  ...
  <header>
    <div class="header-inner">
      <div class="brand">
        <img src="images/logo phoenix-Photoroom.png" alt="شعار مجمع فينكس الطبي" loading="eager" decoding="async">
      </div>
  ...
  <main class="page">
    ...
    <section class="hero-strip" aria-label="مقدمة">
      <div class="hero-title">
        <div>
          <h1><i class="fa-solid fa-sparkles"></i> حجز موعد — مجمع فينكس الطبي</h1>
          <p class="hero-sub">تسجيل دخول عبر واتساب ثم اختيار العيادة والوقت بكل سهولة.</p>
        </div>
    ...
    <!-- تسجيل الدخول -->
    <section id="loginSection" class="card" aria-label="تسجيل الدخول">
      <div class="card-head">
        <span class="chip"><i class="fa-solid fa-user-check"></i> تسجيل الدخول</span>
      ...
      <div class="note" style="margin-top:12px;">
        🔔 تنبيه: خدمات الليزر متاحة للحجز داخل المجمع فقط. <br>
        للتواصل اضغط أيقونة الواتساب أعلى الصفحة.
      </div>
      ...
      <div class="row" style="margin-top:10px;">
        <button class="btn btn-ghost" id="openNewFileBtn" type="button" style="flex:1;">
          <i class="fa-solid fa-folder-plus"></i> لا تملك ملفًا؟ افتح ملف جديد
        </button>
      </div>
    </section>

    <!-- الحجز -->
    <section id="bookingSection" class="card" style="display:none;" aria-label="الحجز">
      <div class="card-head">
        <span class="chip"><i class="fa-solid fa-calendar-check"></i> الحجز</span>
        

        <button id="toggleHours" type="button" class="btn-hours">
          <i class="fa-solid fa-clock"></i> ساعات العمل
        </button>
      </div>
  ...
  <option value="">اختر العيادة</option>
          ...
          <!-- ملاحظة خاصة لعيادة تنظيف البشرة فقط -->
...
<option value="12">ديسمبر</option>
          ...
          <button id="fetchBtn" class="btn btn-green" style="margin-top:10px;">
            <i class="fa-solid fa-magnifying-glass"></i> عرض المواعيد
          </button>
        </div>
      </div>
  ...
  <button id="logoutBtn" class="btn btn-danger">
    ...
    <!-- ساعات العمل -->
    <section id="hoursSection" class="card" style="display:none;" aria-label="ساعات العمل">
      <div class="card-head">
        <span class="chip"><i class="fa-solid fa-clock"></i> ساعات العمل</span>
        <button id="toggleBooking" type="button" class="btn-hours">
          <i class="fa-solid fa-calendar-check"></i> الرجوع للحجز
        </button>
      </div>
      ...
      <div style="overflow:auto;">
        <table class="hours-table">
          ...
          <tbody>
            ...
            <tr>
  <td>عيادة الدكتورة هاجر</td>
  <td>السبت - الخميس</td>
  <td>
    الفترة الصباحية: 9:00 ص – 11:00 ص<br>
    الفترة المسائية: 3:00 م – 9:00 م
  </td>
  <td></td>
</tr>

<tr>
  <td>عيادة الدكتور مآب</td>
  <td>الخميس فقط</td>
  <td>
    الفترة المسائية: 2:00 م – 10:00 م
  </td>
  <td></td>
</tr>
...
<script>
  ...
  // تفعيل زر تسجيل الدخول (يدعم أرقام عربية وإنجليزية)
...
async function loadClinics() {
  try {
    ...
    // تفريغ القائمة
    clinicSel.innerHTML = '<option value="">اختر العيادة</option>';

    data.clinics.forEach(c => {
      const opt = document.createElement('option');
      opt.value = c.value; // نفس القيمة المستخدمة في الحجز
      opt.textContent = `${c.label} (${c.from} – ${c.to})`;
  ...
  // إخفاء الشهر وزر عرض المواعيد (لم يعدا مستخدمين)
monthSel.closest('.field').style.display = 'none';
fetchBtn.style.display = 'none';

// قيمة شكلية فقط للتوافق
monthSel.value = 'auto';
  ...
  // >>> دالة صغيرة لتخزين لقطة بيانات الحجز لصفحة success.html
  ...
  // دالة لتفريغ النتائج وإجبار إعادة العرض
  function resetTimes(trigger=''){
   timeSel.innerHTML = `<option value="">اختر عيادة لعرض الأوقات</option>`;
timeSel.disabled = false;   // 🔥 لا تعطلها
bookNowBtn.disabled = true;
  ...
  // إظهار الملاحظة الخاصة لعيادة تنظيف البشرة فقط + إظهار خانة "نوع الخدمة/ملاحظة" عند اختيار أي عيادة
clinicSel.addEventListener('change', ()=>{
  const v = clinicSel.value || '';
  const isCleaning = v.startsWith('عيادة تنظيف البشرة');
  ...
  // 🔥 اجلب المواعيد تلقائيًا
loadClinics();



  // تفريغ الأوقات عند تغيير الشهر
  ...
  fetchBtn.addEventListener('click', async () => {
    ...
    if (!clinic) {
  timeSel.innerHTML = `<option value="">يرجى اختيار العيادة</option>`;
  return;
}
...
const phone       = mobile.value.trim();
  ...
  bkMsg.textContent = 'الرجاء اختيار العيادة والوقت وتسجيل الدخول.';
  return;
}
  ...
  // ===== تبديل العرض بين الحجز وساعات العمل =====
